...
Partial Public Class kolej_view
    ...
    Private Sub LoadPage()
        strSQL = "SELECT * FROM kpmkv_kolej WHERE RecordID='" & lblKolejID.Text & "'"
        Dim strConn As String = ConfigurationManager.AppSettings("ConnectionString")
        Dim objConn As SqlConnection = New SqlConnection(strConn)
        ...
        Try
            ...
            If MyTable.Rows.Count > 0 Then
                ...
                If Not IsDBNull(ds.Tables(0).Rows(0).Item("JawatanPengarah")) Then
                    lblJawatanPengarah.Text = ds.Tables(0).Rows(0).Item("JawatanPengarah")
                Else
                    lblJawatanPengarah.Text = ""
                End If
                If Not IsDBNull(ds.Tables(0).Rows(0).Item("GredPengarah")) Then
                    lblGredPengarah.Text = ds.Tables(0).Rows(0).Item("GredPengarah")
                Else
                    lblGredPengarah.Text = ""
                End If
                ...
                If Not IsDBNull(ds.Tables(0).Rows(0).Item("JawatanKJPP")) Then
                    lblJawatanKJPP.Text = ds.Tables(0).Rows(0).Item("JawatanKJPP")
                Else
                    lblJawatanKJPP.Text = ""
                End If
                If Not IsDBNull(ds.Tables(0).Rows(0).Item("GredKJPP")) Then
                    lblGredKJPP.Text = ds.Tables(0).Rows(0).Item("GredKJPP")
                Else
                    lblGredKJPP.Text = ""
                End If
    ...
    Protected Sub btnExecute_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnExecute.Click
        Response.Redirect("kolej.update.aspx?RecordID=" & lblKolejID.Text)
    End Sub
